...
# Prefer the ticket file that actually holds the acceptance criteria.
# Phase docs (build/review/learn) may MENTION "Acceptance Criteria" in prose —
# substring grep hijacks the preference and produces a bogus 0-AC "pass".
# Count real checkbox ACs under the AC heading; take the richest file.
NAME_FILE=""
BEST_COUNT=0
for f in $(find "$SCRIPT_DIR/aes/tickets" -name "${TICKET}-*.md" 2>/dev/null); do
  cnt=$(awk '
    /^#+[[:space:]]*(Acceptance Criteria|Critérios de aceitação)[[:space:]]*$/ { insec=1; next }
    /^#/ && insec { exit }
    insec && /- \[[ x]\] / { n++ }
    END { print n+0 }
  ' "$f")
...
while IFS= read -r line; do
  case "$line" in
    *"Acceptance Criteria"*|*"Critérios de aceitação"*) IN_SECTION=1; continue ;;
    "##"*) [ "$IN_SECTION" = "1" ] && break ;;
  esac
