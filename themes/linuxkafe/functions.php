...
/* ========================================================= */
/* GAMIFICATION SCRIPTS (T001)                               */
/* ========================================================= */
function linuxkafe_enqueue_gamification() {
    // Only load on frontend, not in admin/customizer preview unnecessarily
    ...
    // Core gamification module (ESM) - loads all features
    wp_enqueue_script(
        'linuxkafe-gamification',
        $js_dir . '/gamification.js',
        array(),
        filemtime( get_template_directory() . '/js/gamification.js' ),
        true // in footer
    );

    // Add type="module" for ESM support (WP 6.3+)
    // Fallback for older WP: add type="module" via script_loader_tag filter
    if ( function_exists( 'wp_script_add_data' ) && version_compare( get_bloginfo( 'version' ), '6.3', '>=' ) ) {
        wp_script_add_data( 'linuxkafe-gamification', 'type', 'module' );
    } else {
        add_filter( 'script_loader_tag', function( $tag, $handle ) {
            if ( 'linuxkafe-gamification' === $handle ) {
    ...
    // Pass config to JS via wp_localize_script (for feature flags, etc.)
    // Read Customizer theme_mods with fallback to true
    $gamification_enabled = get_theme_mod( 'linuxkafe_gamification_enabled', true );
    $features_config = array(
        'shell'      => $gamification_enabled && get_theme_mod( 'linuxkafe_gamification_shell', true ),
        'tux'        => $gamification_enabled && get_theme_mod( 'linuxkafe_gamification_tux', true ),
        'graffiti'   => $gamification_enabled && get_theme_mod( 'linuxkafe_gamification_graffiti', true ),
        'cybercafe'  => $gamification_enabled && get_theme_mod( 'linuxkafe_gamification_cybercafe', true ),
        'csHud'      => $gamification_enabled && get_theme_mod( 'linuxkafe_gamification_easter_eggs', true ),
        'easterEggs' => $gamification_enabled && get_theme_mod( 'linuxkafe_gamification_easter_eggs', true ),
    );

    wp_localize_script( 'linuxkafe-gamification', 'lkGamificationConfig', array(
        'features' => $features_config,
        'timing' => array(
            'cybercafeMinInterval' => 30000,
            'cybercafeMaxInterval' => 60000,
            'cybercafeMaxVisible'  => 3,
            'cybercafeAutoDismiss' => 8000,
            'graffitiCycleDuration' => 15000,
            'tuxIdleThreshold'     => 10000,
            'shellAnimationDuration' => 300,
        ),
        'debug' => defined( 'WP_DEBUG' ) && WP_DEBUG,
    ) );

    // Analytics config
    wp_localize_script( 'linuxkafe-gamification', 'lkAnalyticsConfig', array(
        'plausible' => defined( 'LK_PLAUSIBLE_DOMAIN' ),
        'ga4' => defined( 'LK_GA4_MEASUREMENT_ID' ),
        'customEndpoint' => rest_url( 'linuxkafe/v1/analytics' ),
    ) );
}
...
add_action( 'rest_api_init', function() {
    register_rest_route( 'linuxkafe/v1', '/analytics', array(
        'methods'  => 'POST',
        'callback' => 'linuxkafe_analytics_endpoint',
...
function linuxkafe_analytics_endpoint( $request ) {
    ...
    $log_file = WP_CONTENT_DIR . '/linuxkafe-analytics.log';
